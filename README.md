...
- **Screenshot scan** — snip the hero-select/scoreboard screen (`Win+Shift+S`), press `Ctrl+V` on the page (or upload a file). The image is resized client-side (max 1400px), optionally cropped to the enemy column (Right/Left/Full toggle), and sent to Claude's vision model to auto-fill the picks.
